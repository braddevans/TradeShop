...
public class JsonConfiguration extends Utils implements Serializable {
    ...
    public JsonConfiguration(Chunk c) {
        ...
        if (! this.file.exists()) {
            ...
            if (new File(path + File.separator + chunk.serialize().replace(";;", "_") + ".json").exists()) { this.file = new File(path + File.separator + chunk.serialize().replace(";;", "_") + ".json"); }
